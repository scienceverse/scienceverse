...
This is what is created. You won't usually need to look at it, but it's just a nested list that we can then save in JSON format later.

```{r, echo=FALSE}
study_json(study) %>% cat()
```
...
```{r, echo=FALSE}
study_json(list(authors = study$authors)) %>% cat()
```
...
```{r, echo=FALSE}
study_json(list(hypotheses = study$hypotheses)) %>% cat()
```
...
```{r, echo=FALSE}
study_json(list(analyses = study$analyses)) %>% cat()
```
...
```{r, echo=FALSE}
study_json(list(criteria = study$hypotheses[[1]]$criteria)) %>% cat()
```
...
```{r, echo=FALSE}
study_json(list(corroboration = study$hypotheses[[1]]$corroboration,
                falsification = study$hypotheses[[1]]$falsification)) %>% cat()
```
...
```{r, echo=FALSE}
study_json(list(data = study$data)) %>% cat()
```
