...
study_save(study, "files/prereg.json", "json")
```

You can view the [JSON-formatted study description](files/prereg.json)  through: `file.edit('files/prereg.json')`.
...
```{r}
study_save(study, "files/prereg.html", "prereg")
```

View the [prereg](files/prereg.html) report.
...
If you don't have existing data, you can simulate data with factorial designs with the `add_sim_data` function, which uses the same syntax as `sim_design` from [faux](https://debruine.github.io/faux/). Give the simulated data the same `data_id` as referenced in the analyses to test your analysis code.

```{r, eval = FALSE}
# this function currently only simulates data with a normal distribution
study <- add_sim_data(study, 
                      data_id = "kin",
                      within = list(
                        game = c("trust", "recip"),
                        opponent = c("self", "other")
                      ),
                      n = 24,
                      mu = list(trust_self = 1.3, 
                             trust_other = 0.8,
                             recip_self = 1.8,
                             recip_other = 1.8),
                      sd = c(0.8, 0.8, 1.1, 1.1),
                      r = .5)
```
...
When you run `study_analyze` (or `stdy_analyse`), the data are loaded as their `id` names and the analyses are run in order. The evaluation of each criterion and hypothesis is printed as a message.

```{r}
study <- study_analyze(study)
```
...
```{r}
study_save(study, "files/postreg.json", "json")
study_save(study, "files/postreg.html", "postreg")
```

View the [postreg](files/postreg.html) report.

You can view the [JSON-formatted study description](files/postreg.json) through: `file.edit('files/postreg.json')`.

### Reload

You can load a study directly from the JSON file with the `study` function.

```{r}
study2 <- study("files/postreg.json")
```

Use the function `get_data` to extract data from the study object.

```{r}
kindata <- get_data(study2, "kin")
```

### View sections

You can include sections of the human-readable summaries in any report or just view them in the console with the functions `output_info`, `output_hypotheses`, `output_analyses` and `output_results`. By default, they output markdown (`md`), but you can change that to `html` or plain `text`. If you want to display markdown or html from a code chunk, make sure to set `results='asis'`. The `header_lvl` argument determines what level the first header is (defaults to 2).

```{r, results='asis'}
output_info(study2, header_lvl = 4)
```

```{r, results='asis'}
output_hypotheses(study2, header_lvl = 4, output = "md")
```

```{r, results='asis'}
output_analyses(study2, header_lvl = 4, output = "html")
```

```{r}
output_results(study2, header_lvl = 4, output = "text")
```
