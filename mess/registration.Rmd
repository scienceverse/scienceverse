...
#devtools::install_github("debruine/scienceverse/reg")
...
You can install the released version of pipeline from [GitHub](https://github.com/debruine/scienceverse/reg) with:
...
```{r}
apathy_depression_study <- add_hypothesis(apathy_depression_study, "The correlation between the apathy and depression scale is smaller than 0.3")
```
...
#READ RAW DATA
apathy_depression_data <- read.csv("apathy_depression.csv")
# 
# data$apathy <- (data$a1 + data$a2 + data$a3 + data$a4 + data$a5)/5
# data$depression <- (data$d1 + data$d2 + data$d3 + data$d4 + data$d5)/5
# #WRITE ANALYSIS DATA
# write.csv(data, "apathy_depression.csv", row.names = FALSE)

source("eq_test_r.R")
res <- eq_test_r(data = apathy_depression_data, "apathy", "depression", alpha = 0.05, high_eqbound_r = 0.3, low_eqbound_r = -0.3)

apathy_depression_study <- add_analysis(apathy_depression_study, 
                                       func = "eq_test_r",
                                       params = list(
                                         data = ".data[1]", #when entering .data it gave an error? object '.data' not found - because I already named something data?
                                         col1 = "apathy",
                                         col2 = "depression",
                                         alpha = 0.05,
                                         high_eqbound_r = -0.3,
...
apathy_depression_study <- add_criterion(apathy_depression_study, 
                            result = "TOST_p2", 
                            operator = "<", 
                            comparator = 0.01) #maybe rename comparator?
```
...
study_save(apathy_depression_study, "apathy_depression.json")
```
...
#Lisa, this gave an error because eq_test_r was not defined: 
#The function eq_test_r in analysis 1 is not defined
#I had to first load the function. The code could do this, and give an error if it does not load? 
str(apathy_depression_study)
```

## Preregistering your hypothesis and analysis plan.

Because we specified our test and evaluation criteria for our prediction in detail in the JSON file, we can automatically extract this information, and summarize it in a human-readable format that can be used to preregister our statistical prediction. 

We can do this by creating a summary of the JSON file that contains the sections that are relevant for the preregistration. In this case, it means running `study_report` command, asking for the 'prereg' template. This will return the output below, with a hypothesis section and an analysis section. 

----------------------------------------------------------------

```{r, results='asis'}
study_report(apathy_depression_study, template = "prereg", filename = "prereg_apathy_depression_study.html")

#Lisa, the analysis in the html file does not look good:
#
#eq_test_r(data = function (..., list = character(), package = NULL, lib.loc = NULL, , col1 =     verbose = getOption("verbose"), envir = .GlobalEnv) , col2 = {, alpha =     fileExt <- function(x) {, high_eqbound_r =         db <- grepl("\\.[^.]+\\.(gz|bz2|xz)$", x), low_eqbound_r =         ans <- sub(".*\\.", "", x))
```
...
```{r}
#There is a ' code' entry in the analysis list - is that supposed to have the code? It is empty now. 
```
